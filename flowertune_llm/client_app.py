...
class FlowerClient(NumPyClient):
    ...
    def __init__(
        ...
        self.calibration_text_column = calibration_text_column
    ...
    def fit(
        ...
        if config['current_round']==1:
            ...
            calibration_size = min(
                self.calibration_cfg.candidate_rows, len(self.trainset)
            )
            calibration_set = self.trainset.select(range(calibration_size))
            line_scores, element_scores = evaluate_lora_importance(
                self.model,
                calibration_set,
                self.tokenizer,
                self.task_type,
                self.actual_task,
                self.device,
                text_column=self.calibration_text_column,
                nsamples=self.calibration_cfg.num_samples,
                seed=self.calibration_cfg.seed,
            )
            ...
            global_enc_lines = pickle.loads(config['enc_lines'])
            self.enc_lines = set_enc_lines_to_client(global_enc_lines,self.he_budget)
            if self.model_cfg.task_type == "NLU":
    ...
    def trainer_loader(self):
        if self.task_type == 'NLG':
            ...
            raise ValueError("Only NLG training with local Dolly data is supported.")
...
def client_fn(context: Context) -> FlowerClient:
    ...
    if cfg.model.task_type == "NLG":
        client_trainset = load_data(partition_id, num_partitions, cfg.dataset)
        (
            tokenizer,
            data_collator,
            formatting_prompts_func,
        ) = get_tokenizer_and_data_collator_and_propt_formatting(
            cfg.model.path, cfg.dataset
        )
        ...
        return FlowerClient(
            ...
            calibration_text_column=cfg.dataset.instruction_column,
            calibration_cfg=cfg.calibration,
        ).to_client()
    raise ValueError("The local configuration supports NLG with Dolly only.")
